...
my_font3 = pygame.font.SysFont('Consolas', 15)
my_font4 = pygame.font.SysFont('Consolas', 11)
pygame.display.set_caption('Escalate')


# -General/game
fps = 120
mouseclick_check = False
capper = 0
...
started = False
opening_screen = True
ability_screen_main_1 = False
ability_screen_passive_1 = False
ability_screen_main_2 = False
ability_screen_passive_2 = False
...
Railgun_lvl = [3, 3]
Railgun_toggle = [True, False]
R_bounce_prt1 = [False, False]
R_bounce_prt2 = [False, False]
railgun_cost = 40
railgun_getpotential = [False, False]
...
slow_effect = False
blink_cost = 30
blink_getpotential = [False, False]
...
shield_getpotential = [False, False]
...
airjump_cost = 35
airjump_counter = [0, 0]
airjump_getpotential = [False, False]
...
combo_lvl = [3, 3]
combo_cost = 40
combo_getpotential = [False, False]
...
randomize_onoffquick = [True, True]
randomize_getpotential = [False, False]

regenerative_pl = [True, True]
regenerative_toggle = [False, False]
regenerative_counter = [0, 0]
regenerative_lvl = [2, 2]
regenerative_cost = 40
regenerative_getpotential = [False, False]


def mouseclick(event):
    global mouseclick_check, capper

    mouseclick_check = False
    if event.type == pygame.MOUSEBUTTONDOWN:
        mouseclick_check = True

def lighten(lighten_max, lighten_spd, xmouse, ymouse, x1, x2, y1, y2, r, g, b, r1, r2, r3):

    if xmouse >= x1 and xmouse <= x2 and ymouse >= y1 and ymouse <= y2:
        if r < lighten_max:
            r += lighten_spd
        if g < lighten_max:
            g += lighten_spd
...
def airjump(direction, x):
    ...
    if airjump_lvl[0] == 2 and x > 850:
        airjump_toggle[0] = False
    if airjump_lvl[0] == 3 and x > 1100:
        airjump_toggle[0] = False

    if airjump_lvl[1] == 1 and x < 650:
        airjump_toggle[1] = False
    if airjump_lvl[1] == 2 and x < 450:
        airjump_toggle[1] = False
    if airjump_lvl[1] == 3 and x < 200:
        airjump_toggle[1] = False
...
class MainBall:
    ...
    def move_circle(self, Pla1, Pla2):
        ...
        if combo_toggle[0] is True and Left is False:
            self.spd = 8 + combo_counter[0]
            print(self.spd)
        if combo_toggle[1] is True and Left is True:
            self.spd = 8 + combo_counter[1]
...
def starting():
    global Menu, started, r1, r2, r3, mouseclick_check, opening_screen, ability_screen_main_1
    pygame.event.pump()
    key = pygame.key
...
def points_dis():
    points1 = my_font3.render(str(points[0]), False, (0, 0, 0))
    points2 = my_font3.render(str(points[1]), False, (0, 0, 0))

    screen.blit(points1, (400 - points1.get_width(), 20))
    screen.blit(points2, (900, 20))
...
while 1:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_f and combo_cost <= player_mana[0] and combo_pl[0] is True:
                combo_toggle[0] = True
        ...
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_RCTRL and combo_cost <= player_mana[1] and combo_pl[1] is True:
                combo_toggle[1] = True
        ...
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_SPACE and regenerative_cost <= player_mana[0] and regenerative_pl[0] is True:
        ...
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_KP_ENTER and regenerative_cost <= player_mana[1] and regenerative_pl[1] is True:
        ...
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_w and player_mana[0] >= railgun_cost and Railgun_Pl[0] is True:
        ...
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_UP and player_mana[1] >= railgun_cost and Railgun_Pl[1] is True:
        ...
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_e and Left is True and player_mana[0] > blink_cost and blink_pl[0] is True:
                if cardinal_dir[0] is True:
                    blink_button[0] = True
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_PAGEDOWN and Left is False and player_mana[1] > blink_cost and blink_pl[1] is True:
